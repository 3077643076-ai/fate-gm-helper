...
## 2026-09-16（夜 20，收工总结）

**今晚做完的事（都已推送）**：
1. **登录链路修通**：`spawn` 引号坑（NapCat 根本没启动）、取码接口没走自动发现目录、失败无反馈、
   后端改绑 127.0.0.1（免掉每次的防火墙弹窗）
2. **C1 落地（本轮需求）**：机器人用**独立 QQ 副本**（`data/qqnt-bot/`）→ 和 GM 自己的 QQ 同时在线 ✓ 实测通过；
   - 免终端（不走带 `pause` 的 launcher bat，直接调 `NapCatWinBootMain.exe`）
   - `detached + napcat.pid`：关工作台不影响机器人 ✓ 实测通过
   - UI 新增「QQ 环境」区块：一键准备独立 QQ / 停止机器人（不动 GM 的 QQ）
   - 合规：NapCat 可非商业随包（已附许可原文）；**腾讯 QQ 不可随包** → 做成"复制本机已装的 QQ"的功能
3. **结算前页行动表**：补 代号（`unit_registry.code`）/ 玩家 QQ 名（新表 `engine_player_binding`，私组群主优先，
   `/engine/players`）/ 原文（`raw_text`）；并写明两张表的分工
4. **二维码新鲜度提示**：页面显示"码生成于 HH:MM（N 分钟前）"，过期或后端不可达时变灰 + 给最新码入口
5. **发行包**：`desktop/圣杯GM工作台-便携版-1.0.0.exe` + `圣杯GM工作台-便携版-含QQ机器人.zip`
   （141MB，871 文件，含内置 NapCat 与许可声明）

**明天接着做的话，优先级建议**：
- [ ] 给机器人扫一次码（副本 QQ 窗口或 WebUI 页）→ 把链路跑绿，验证 new build 的完整流程
- [ ] **"重启机器人需重扫"** 待优化：试试给副本 QQ 指定独立数据目录（QQData 隔离）
- [ ] `CLAUDE.md` 阶段 6 的方案文档里，A（模式配置化）/E（手动粘贴公告降级路径）/F（登录态进 U 盘备份）都还没做
- [ ] exe + 发行包重打后没跑过完整回归（登录→收行动→AI 代收→结算），明天开团前过一遍

**当前环境状态**：工作台跑的是 22:41 的新构建；机器人未登录（3001 未监听）；副本 QQ 6 进程 / GM 自己 QQ 9 进程。
# NOTES.md
...
海豹那样网页扫码、保存账号多方便。" —— 这三条**与选协议端无关**，都在现有架构内可解：

1. **弹终端**：走的是 `cmd /c launcher-user.bat`，而该 bat 末尾有 `pause` → 控制台窗口一直挂着。
   修法：绕开 bat，直接调 `NapCatWinBootMain.exe <QQ路径> <Hook.dll> <QQ号>`（参数已由 launch.log 实测确认），
   自己写 `loadNapCat.js`，输出落日志文件 → 永不弹窗。
2. **重启工作台会连带杀机器人**：NapCat 目前是后端的子进程，重启时被进程树一起收（今晚误杀两次即此因）。
   修法：`detached + hidden` 放飞成独立进程 + `data/napcat/napcat.pid` 记 PID；UI 给启动/停止/重启按钮，
   停止按 PID 精确停（不再 taskkill 进程树）。
3. **网页扫码 + 记住账号**：其实已经具备（二维码读 `cache/qrcode.png` 在网页显示、登录态存
   `data/napcat/config/`、下次用 `-q <QQ>` 快速登录）→ 只需补账号状态面板与"退出登录/换账号"。

以上写进 `docs/QQ机器人登录方案.md` 第 0 节（标为优先项），约 40 分钟可完成；用户定"今天先不做"。
...
- 社区按作者分系：溯洄系 = **Dice!**（作者 w4123/溯洄，酷Q 时代最有名；官骰 Kokona，
  现称《炼骰手册》Dice! 2.x；另有 DiceZone 的 Dice-Next 重构版）、风羽系 = **CatDice**、
  塔系 = **塔骰**（斯塔尼亚），海豹还兼容**赵骰**
- 除海豹外这些核心都只做骰子逻辑，QQ 登录靠外部协议端；酷Q/Mirai 时代走**手机/平板协议**，
  所以"骰娘在线 + 自己在 QQ 上"当年不是问题（炼骰手册原话），现在协议端收敛到 NTQQ PC 端协议才挤下线
- 海豹的思路 = **核心与协议端一体打包**：v1.5.1 起自带 `Lagrange.Milky` / `Yogurt`，
  自动生成本机 Milky 地址/令牌/配置，用户只做"填号 + 扫码"；且兼容溯洄/赵骰/塔骰主流指令
- 结论：我们不需要骰子核心的功能，要选的是**协议端**；海豹验证过的组合就是 Lagrange.Milky / Yogurt

**下一步（用户自己先测海豹）**：完整发布包装海豹 → 账号设置加 QQ → 连接方式「内置 Yogurt」→
填机器人号扫码 → **重点看"自己的 QQ 保持在线时机器人能否同时在线"**；结果回填 CLAUDE.md 阶段 6，
成立就走方案 C（给 `backend-node` 加 Milky 协议适配）。
