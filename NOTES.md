...
- GM 需求：用尽简写/俗称/谐音糊弄规范化，测 196 条（14 单位×14 行动类）；克隆三国
  战役做测试副本；然后接真 QQ 私组实测
- `tools/parser-stress.mjs`：14×14 糊弄矩阵跑 parseAction（只读，安全）。基线
  79/196（40%），失败全是动词不认识（灵脉目标归一零失误）
- `backend-node/scripts/seed-alias-pack.mjs`：30 条别名（位移/移动/去→机动，
  吃饭/吸食/进食→魂食，遮断/挡饭→魂食遮断，恶食/恶性→恶性魂食，无限→无限制魂食，
  干预→干涉，偷袭/夜袭→袭击，广侦/广域侦查/广泛侦查（全图）→广泛侦查，
  调查/情报侦查/查一下→情报调查，资分/资料分柝/分析资料→资料分析，
  猜名/真名推定/猜→真名猜测，休息/休生/苟一轮→休整）
- `seed-action-rules.mjs` 增两条**抽象占位口径**：「技能」「宝具」——
  耗行动力、无数值预设、效果 GM 按原文裁决（规则书里技能/宝具行动的引擎挂靠点）
- 复测 **193/196（98%）**；剩「介入B」连写不救（沟通问题非解析问题）
- `tools/clone-campaign.mjs`：战役克隆（卡/状态/灵脉/分配重映射 id；引擎行动/
  群映射/日志不带走）。已克隆 999002→**999102 三国杯测试副本**（14 卡/10 灵脉/14 分配），
  干跑验证机动/广侦/宝具三路解析全通
- 下一步（GM 侧）：真 QQ 私组实测——设置页填真 NapCat 地址、群映射录真实群号，
  行动统计页「连 NapCat 统计」或 AI 一键收行动

## 2026-09-11（四）

**功能②每轮末魔力统计完成**
- 后端 `GET /api/engine/mana/report?campaignId&round`：每单位一行——
  上轮期末录入（R-1 character_status）/ 行动增减（昼+夜全部非 void 行动 ×
  action_rules 魔力口径，悬停看逐条明细）/ 应然期末 = 期初+增减 / 本轮录入（R）/
  差异 = 录入-应然（≠0 标红）。未 settled 的含魔力行动打「未结算」警示。
  孤儿卡（状态表有、注册表没关联）单列防漏人；mana_limit 取状态行，回退卡面 total_mana
- 前端 `ManaStatsPage.vue`（/mana-stats，导航水滴图标，支持深链 ?campaignId=）：
  对照表 + chips（上轮/本轮录入覆盖数、差异≠0 数、含未结算数）；差异列「平」绿/
  ±N 红；战斗增减列未计入（功能④接入后自动带上，表头有注明）
- 验证：真实库造数据（张角 100+60=160 平；司马师 80-30 征兵=50 平；
  刘协无行动录入差 +20 红）→ API 数值全对 + 无头断言通过 → 测试数据 SQL 清场
- 设计约定：status[R] 语义=「该回合录入值」，期末对照=R-1 录入 vs R 录入夹行动增减；
  转让/手动调整不进引擎账，GM 按差异列人工核对

## 2026-09-11（三）

**v1 功能清单定盘 + 功能①行动统计页**
- GM 拍板第一版只做四个功能，逐个来：①行动统计-规范-排序 ②每轮末魔力统计
  ③战斗统计 ④战斗魔力结算。目标单 goal-85f16df6 盯进度
- **功能①完成**：`ActionStatsPage.vue`（/action-stats，导航第二项）——
  战役/回合/时段选择 + 统计/连 NapCat 统计/催未交/复制规范单四按钮；
  概览 chips（已交组、单位、行动、需裁决、编外单位）；私组提交表（公告状态）；
  主表=规范单按结算链排序（链段分隔行+连续编号+判定率/魔力/状态徽章，
  作废划线，悬停看原文）；复制规范单=纯文本逐行可贴群
- 后端无改动（全部复用 /actions/summary、/notices/remind-missing）
- 页面支持深链 `?campaignId=`（URL > localStorage > 列表首个）——无头验证就靠它
- 踩坑记录：①无头 profile 无 localStorage → 默认选了列表第一个战役（测试杯），
  数据全空——不是 bug，是没指定战役；②Edge --dump-dom 偶发只抓到壳（fetch 未回），
  断言前先看 dump 大小；③DELETE /actions/:id 是置 void 留痕不是物理删（设计如此），
  清测试数据要 SQL
- 修了页面 computed 两个小毛病：序号计数器跨缓存不重置、链段头 O(n²) find
